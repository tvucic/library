...
export function authenticate(accessToken, refreshToken) {
  if (!accessToken || !refreshToken) {
    console.warn(
      `Got bad token while logging in.\nAccess Token: ${accessToken}.\nRefresh Token: ${refreshToken}`
    );
  }
...
export async function login(username, password) {
  ...
  await axios(config)
    .then(function (resp) {
      ...
      let accessToken =
        "access_token" in resp.data ? resp.data.access_token : null;
      let refreshToken =
        "refresh_token" in resp.data ? resp.data.refresh_token : null;
  ...
  axios(config)
    .then(function (response) {
      if (response.status === 200) {
        ...
        console.log(
          "SYSTEM: Access token was taken, system are trying to login with the access token..."
        );
        let accessToken =
          "access_token" in response.data ? response.data.access_token : null;
        authenticate(accessToken, refreshToken);
      }
    })
...
export async function getAccessToken() {
  ...
  if (accessToken && loggedBefore <= new Date().getTime()) {
    console.log(
      "SYSTEM: Access token expired, system are trying to refresh the access token..."
    );
